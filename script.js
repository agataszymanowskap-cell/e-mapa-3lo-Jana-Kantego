...
function zoomToPlace(place){
    ...
    svg.querySelectorAll("path").forEach(path => {
        ...
        if(path.outerHTML.includes(place.id)){
    ...
    let screenCenterX = svgBox.width / 2;
    let screenCenterY = svgBox.height / 2;
...
let transform =
    `translate(${screenCenterX + offsetX} ${screenCenterY + offsetY})
     scale(${zoom})
