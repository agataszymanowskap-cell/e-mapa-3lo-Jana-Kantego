...
console.log(
"WSZYSTKIE PATH Z NAZWAMI:",
Array.from(svg.querySelectorAll("path"))
.map(path => ({
data: path.getAttribute("data-layer-name"),
vector: path.getAttribute("vectornator:layerName")
}))
.filter(x => x.data || x.vector)
);
...
path.getAttribute("data-layer-name") || ""
)
.trim()
.toLowerCase();

if(layerName === pointName){
...
// =================================
// DODATKOWY WARUNEK TYLKO DLA AULI
// =================================
...
// POZOSTAŁE MIEJSCA Z PARTERU → SCHODY2

else if(
startPlace &&
startPlace.floor === "parter"
){
...
if(endPlace.floor === "parter"){

stairName = floorChange.stair;

}
else if(pendingRoute.end === "aula"){
